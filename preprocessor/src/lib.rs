    ...
    for data in images {
        ...
        for x in 0..data.width {
            for y in 0..data.height {
                ...
                let dst_idx = ((((y >> 3) * (data.width >> 3) + (x >> 3)) << 6)
                    + ((x & 1)
                        | ((y & 1) << 1)
                        | ((x & 2) << 1)
                        | ((y & 2) << 2)
                ...
                texture.data[dst_idx * 4 + 0] = 255;
                texture.data[dst_idx * 4 + 1] = data.pixels[src_idx * 3 + 2];
                texture.data[dst_idx * 4 + 2] = data.pixels[src_idx * 3 + 1];
                texture.data[dst_idx * 4 + 3] = data.pixels[src_idx * 3 + 0];
            }
        }
